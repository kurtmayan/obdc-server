...
export class SyncService {
  ...
  private async parseExcelAndSync(
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      const id = getCellText(row.getCell(1).value);
      const deviceIdRaw = getCellText(row.getCell(2).value);
      const employeeName = getCellText(row.getCell(3).value);
      const employeeId = getCellText(row.getCell(4).value);
      const logDate = getCellText(row.getCell(5).value);
